...
- Planos: `Plans.kt` (Grátis, Pro, Empresa e regras puras em `PlanRules`), `PlanStore` (plano do aparelho e contador diário do agente; limites só valem com `ENFORCED`, isto é, com o Firebase configurado), `OstieAccount` (login Firebase Google/e-mail, plano pela declaração `ostiePlan`, checkout e portal da Stripe). Servidor: `server/ostie-api` (projeto na Vercel: `/api/checkout`, `/api/portal`, `/api/webhook`, `/api/volta`; testes com `npm test`). Passo a passo em `docs/ASSINATURA.md`.
