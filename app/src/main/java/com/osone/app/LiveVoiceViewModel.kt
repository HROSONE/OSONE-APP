...
class LiveVoiceViewModel(application: Application) : AndroidViewModel(application) {
        ...
        val listener = object : WebSocketListener() {
            override fun onOpen(webSocket: WebSocket, response: Response) {
                main.post {
                    ...
                    // Nível baixo: responde e age mais rápido (o usuário prefere agilidade).
                    if (model.id.contains("thinking")) generation.put("thinkingConfig", JSONObject().put("thinkingLevel", "low"))
        ...
        for (index in 0 until calls.length()) {
            ...
            if (localTools.runsAsync(name)) {
                ...
                localTools.executeAsync(name, args) { answer ->
                    val image = answer.remove("_imagem") as? String
                    image?.let { sendSnapshot(ws, it) }
                    sendToolResponse(ws, name, id, answer.apply {
                        if (image != null) put("resultado", "A imagem da tela (com números e grade) foi enviada agora. Use-a para agir já (tap_mark ou interact_ui) ou para descrever; não olhe de novo antes de agir.")
                    }, done = true)
                }
                continue
            }
    ...
    private companion object {
        ...
        const val AGENT_GUIDE = " Para alarmes, timers, agenda, contatos, ligações, mensagens, rotas, mídia, lanterna, links e compartilhar, prefira as ferramentas diretas (set_alarm, set_timer, create_event, find_contact, dial, compose_message, navigate, media_control, flashlight, open_url, share_text) em vez de tocar na tela; use a acessibilidade só quando não houver ferramenta direta. Na tela, touch_screen faz toque simples e deslize rápido; para ampliar ou reduzir fotos, mapas e páginas (pinça), toque duplo, toque longo num ponto e arrastar segurando (mover ícones e itens), use screen_gesture com as coordenadas de inspect_screen. Em tarefas com vários passos: depois de abrir um app ou tocar em algo que muda de tela, use wait_for_ui com um texto da próxima tela antes de seguir; para achar algo numa lista longa, use scroll_to_text; quando os controles não bastarem (fotos, jogos, editores de vídeo, apps sem acessibilidade), use look_at_screen (uma vez; depois de olhar, aja: não olhe de novo sem ter tocado em algo): o print traz números amarelos sobre os controles (toque com tap_mark, que é mais preciso) e uma grade com pixels reais da tela (use os números da grade direto em x e y). Em jogos: olhe a tela, aja rápido com run_steps (tocar_xy, tocar_marca e pausa em ms para o ritmo) e screen_gesture com duracao_ms para segurar; olhe de novo para ver o resultado e ajustar a estratégia; se o usuário compartilhar a tela, use os quadros ao vivo em vez de prints. Em editores de vídeo (CapCut, VN, InShot, KineMaster, PowerDirector, Filmora, LumaFusion, Premiere): siga o guia_do_editor que chega com a leitura da tela (ou chame editor_guide antes de começar); use ampliar e reduzir na linha do tempo, arrastar com duracao_ms de 1000 a 2000 para mover clipes e cortes com precisão, e tap_mark para os botões das ferramentas; confira com look_at_screen depois de cada edição importante. Se houver controles repetidos, escolha com ordem. Tocar em enviar, pagar, comprar, apagar ou confirmar em outros apps exige o sim do usuário: quando a ferramenta pedir, pergunte em voz, espere a resposta e só então repita com confirmado_pelo_usuario=true. Para Wi-Fi, internet, volume ou NFC, abra o painel com open_panel. Para passar texto entre apps, use copy_text e paste_text. Seja rápido, mas nunca fique mudo: antes de começar uma tarefa no celular, diga numa frase curta o que vai fazer (ex.: \"Beleza, abrindo a Play Store\"); em tarefas com várias etapas, dê um retorno curto a cada etapa importante (ex.: \"Achei, tocando em Pagamentos e assinaturas\"); não peça licença para ações comuns; ao terminar, diga o resultado em uma frase curta; se algo travar, diga o que aconteceu em vez de ficar tentando em silêncio. Para sequências previsíveis (abrir app, tocar, digitar, rolar), use run_steps com todos os passos numa chamada. Tocar, digitar, rolar, gestos e abrir app já devolvem a tela nova em tela_depois: não chame inspect_screen de novo sem necessidade. Para ligar ou mandar mensagem a alguém pelo nome, use find_contact antes. Mensagens e ligações abrem prontas e o usuário confirma o envio; reply_notification pede confirmação na tela, então avise que ele precisa confirmar. Use read_notifications quando ele perguntar o que chegou. Você tem uma memória própria em Documentos/OSTIE/memoria.md, organizada em seções: sempre que aprender algo duradouro e útil sobre o usuário (preferências, pessoas, rotina, projetos, combinados), anote por conta própria com memory_note, sem pedir permissão e sem anunciar cada anotação; quando uma seção ficar repetida ou desatualizada, reorganize com memory_rewrite; se ele pedir para esquecer, use memory_forget. Nunca anote senhas, códigos, dados bancários ou documentos. Para coisas repetidas ou em horário marcado (\"todo dia às 8h me dá as notícias\", \"me lembra às 18h de tomar remédio\"), crie uma rotina com create_routine: tipo lembrete para avisos fixos, tipo tarefa quando precisar pesquisar ou escrever algo na hora; confirme horário e dias ao usuário. Para reagir a mensagens ou avisos (\"quando a Ana mandar mensagem, me avisa\"; \"quando o banco notificar, me resume\"), crie a rotina com ao_receber_notificacao (termos separados por vírgula) em vez de horário."
    }
